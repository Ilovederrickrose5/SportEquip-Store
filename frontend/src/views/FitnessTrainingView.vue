<template>
  <div class="fitness-training-page">
    <!-- 顶部搜索和返回按钮 -->
    <div class="page-header">
      <div class="header-container">
        <SearchBar 
          v-model="searchQuery" 
          placeholder="搜索健身训练装备..."
          @search="handleSearch"
        />
...
<script>
  ...
  computed: {
    ...
    categoryOptions() {
      return this.subCategories
        .filter(cat => cat && cat.main_category_id === 3)
    ...
    allFilteredProducts() {
      return this.products.filter(product => {
        ...
        // 分类筛选（基于subCategoryId）
        if (this.selectedCategories.length > 0) {
          // 确保商品有二级分类ID
          if (!product.subCategoryId && product.subCategoryId !== 0) {
            return false;
          }
          // 将两者都转为字符串进行比较，确保类型完全匹配
          const productCategoryIdStr = String(product.subCategoryId);
          // 检查商品的二级分类ID是否在用户选择的分类中
          const matches = this.selectedCategories.some(catId => 
            String(catId) === productCategoryIdStr
          );
          return matches;
        }
        
        // 品牌筛选
        if (this.selectedBrands && product.brand !== this.selectedBrands) {
          return false;
        }
  ...
  methods: {
    ...
    async loadData() {
      ...
      try {
        ...
        this.products = Array.isArray(allProducts) 
          ? allProducts.filter(product => product.mainCategoryId === 3)
          : [];
        
        this.loadingStatus = `成功加载 ${this.products.length} 个健身训练商品`;
        ...
        // 为了确保页面能显示数据，暂时使用模拟数据
        this.simulateApiResponse();
        this.loadingStatus = '使用模拟数据: ' + error.message;
      } finally {
        this.loading = false;
      }
    },
      ...
      products.forEach(product => {
        ...
        if (product.subCategoryId && product.subCategoryName) {
          uniqueSubCategories[product.subCategoryId] = {
            ...
            main_category_id: product.mainCategoryId || 3 // 添加主分类ID，默认为3（健身训练）
          };
        }
    ...
    simulateApiResponse() {
      // 使用数据库中已有的分类数据
      const subCategories = [
        { id: 2, name: '徒步装备', main_category_id: 3 },
        { id: 14, name: '力量训练', main_category_id: 3 },
        { id: 15, name: '有氧训练', main_category_id: 3 },
        { id: 16, name: '健身服装', main_category_id: 3 }
      ];
      
      const brands = [
        { id: 1, name: 'Nike', sub_category_id: 2 },
        { id: 2, name: 'Adidas', sub_category_id: 2 },
        { id: 3, name: 'Under Armour', sub_category_id: 14 },
        { id: 4, name: 'Lululemon', sub_category_id: 16 },
        { id: 5, name: 'Reebok', sub_category_id: 15 },
        { id: 6, name: 'Puma', sub_category_id: 16 }
      ];
      ...
      for (let i = 0; i < totalItems; i++) {
        ...
        const categoryIndex = Math.floor(Math.random() * subCategories.length);
        
        mockProducts.push({
          id: i + 1,
          name: `${brands[brandIndex].name} ${subCategories[categoryIndex].name}装备 ${i + 1}`,
          description: `专业训练装备，提供舒适体验和卓越性能`,
          price: Math.floor(Math.random() * 1000) + 100, // 100-1100元
          imageUrl: null,
          mainCategoryId: 3, // 健身训练主分类ID
          subCategoryId: subCategories[categoryIndex].id,
          subCategoryName: subCategories[categoryIndex].name,
          thirdCategoryId: brands[brandIndex].id,
          thirdCategoryName: brands[brandIndex].name
        });
      }

      this.products = mockProducts;
      this.subCategories = subCategories;
      this.thirdCategories = brands;
      this.loadingStatus = `使用模拟数据，共 ${this.products.length} 个商品`;
    },
